...
def render_field_positions(memory, template_id):
    ...
    for idx, (field_name, regions) in enumerate(template.fields.items()):
        ...
        for region in regions:
            ...
            fig.add_shape(
                type="rect", x0=x0, y0=1 - y1, x1=x1, y1=1 - y0,
                line=dict(color=color, width=2),
                fillcolor=color, opacity=0.3,
            )
            fig.add_annotation(
                x=(x0 + x1) / 2, y=1 - y0 + 0.01,
                text=field_name, showarrow=False,
    ...
    fig.update_layout(
        ...
        width=700, height=800, showlegend=False,
    )
...
def render_health_dashboard(memory):
    ...
    if summary.templates_by_health_grade:
        ...
        fig = go.Figure(data=[go.Bar(
            ...
            marker_color=[
                "#2ca02c" if g == "excellent" else
                "#1f77b4" if g == "good" else
                "#ff7f0e" if g == "developing" else "#d62728"
    ...
    if summary.templates_ranked:
        ...
        st.dataframe(summary.templates_ranked, use_container_width=True, hide_index=True)
...
def render_template_detail(memory, template_id):
    ...
    stability_data = [
        {"Field": name, "Stability": f"{score:.3f}", "Status": "✓ Stable" if score > 0.8 else "⚠️ Unstable"}
    ...
    fig = go.Figure(data=[go.Bar(
        ...
        marker_color=["#2ca02c" if float(d["Stability"]) > 0.8 else "#ff7f0e" for d in stability_data],
    )])
    fig.update_layout(title="Field Stability", xaxis_title="Field", yaxis_title="Score", yaxis=dict(range=[0, 1]), height=350)
    st.plotly_chart(fig, use_container_width=True)
...
def render_drift_analysis(memory, template_id):
    ...
    st.markdown("Compare a **normal** document vs a **drifted** one against the stored template.")
    ...
    if drift_drifted.field_drifts:
        ...
        drift_data = [
            {"Field": fd.field_name, "Drift Score": f"{fd.drift_score:.4f}", "Drifting": "⚠️" if fd.is_drifting else "✓"}
            for fd in sorted(drift_drifted.field_drifts, key=lambda x: x.drift_score, reverse=True)
        ]
...
def render_cluster_view(memory, template_id):
    ...
    col3.metric("Newest Record", cluster_stats.newest_record[:10] if cluster_stats.newest_record else "N/A")

    members = memory.get_cluster_members(template_id)
    if members:
        member_data = [
            {"Doc ID": m.doc_id, "Confidence": f"{m.confidence:.3f}", "Recorded At": m.recorded_at[:19]}
            for m in members
        ]
...
def render_record_document(memory):
    ...
    st.markdown("Record new documents to train spatial memory. Use synthetic generators or upload JSON.")
    ...
    with tab_synth:
        ...
        with col2:
            template_id = st.text_input("Template ID", value="employment-form" if doc_type == "Employment Form" else "invoice")
        ...
        if st.button("Record Documents", type="primary"):
            with st.spinner(f"Recording {num_docs} documents..."):
                for i in range(num_docs):
                    ...
                    memory.record(doc, template_id=template_id, doc_id=f"{doc_id_prefix}-{i+1:03d}")
    ...
    with tab_json:
        ...
        json_template_id = st.text_input("Template ID for JSON upload", value="uploaded-template", key="json_tid")
...
def render_locate_field(memory):
    ...
    st.markdown("Search for a field by name using spatial memory. The system scores candidates by position + name similarity.")
    ...
    with col1:
        ...
        template_id = st.selectbox("Search in Template", templates, key="locate_template")
    with col2:
        doc_type = st.selectbox("Test Document Type", ["Employment Form", "Invoice"], key="locate_type")
        variation = st.slider("Test Doc Variation", 0.001, 0.05, 0.01, 0.005, key="locate_var")

    if st.button("Locate Field"):
        doc = make_employment_form(variation=variation) if doc_type == "Employment Form" else make_invoice(variation=variation)
        ...
        if matches:
            st.success(f"Found {len(matches)} candidate(s) for '{field_name}' in template '{template_id}'")
            match_data = [
                {
            ...
            best = matches[0]
            st.markdown(f"**Best match:** `{' '.join(w.text for w in best.key_value.key)}` "
...
def render_identify_template(memory):
    ...
    st.markdown("Submit a document and the system identifies which stored template it matches based on field layout.")

    tab_gen, tab_upload = st.tabs(["Generate Test Document", "Upload Document JSON"])

    with tab_gen:
        templates = memory.list_templates()
        doc_type_options = ["Employment Form", "Invoice", "Unknown (mixed fields)"] + [f"Template: {t}" for t in templates if t not in ("employment-form", "invoice")]
        doc_type = st.selectbox("Document Type", doc_type_options, key="identify_type")
        variation = st.slider("Position Variation", 0.001, 0.15, 0.02, 0.005, key="identify_var")
        ...
        if st.button("Identify Template", key="identify_gen_btn"):
            if doc_type == "Employment Form":
                ...
                doc = Document(pages=[Page(key_values=[
                    KeyValue(key=[Word("Unknown"), Word("Field")], bbox=BBox(0.1, 0.1, 0.2, 0.03), page=1),
                    KeyValue(key=[Word("Random"), Word("Data")], bbox=BBox(0.5, 0.5, 0.2, 0.03), page=1),
                ])])
            else:
                    ...
                    for field_name, regions in template.fields.items():
                        for region in regions:
                            kvs.append(KeyValue(
                                ...
                                bbox=BBox(
                                    max(0.001, min(0.95, region.bbox["x"] + random.uniform(-variation, variation))),
                                    max(0.001, min(0.95, region.bbox["y"] + random.uniform(-variation, variation))),
                                    region.bbox["width"],
                                    region.bbox["height"],
                                ),
                                page=region.page,
            ...
            if match:
                ...
                st.warning("No template matched. This appears to be an unknown document layout.")
    ...
    with tab_upload:
        ...
        ```
        """)
        uploaded_doc = st.file_uploader("Upload Document JSON", type=["json"], key="identify_upload")

        if uploaded_doc and st.button("Identify Uploaded Document", key="identify_upload_btn"):
            try:
                ...
                if match:
                    ...
                    st.warning("No template matched. This is an unknown document layout.")
            except Exception as e:
                st.error(f"Error: {e}")
...
def render_export_import(memory):
    ...
    if st.button("Export"):
        ...
        if fmt == "json":
            ...
            st.download_button("Download JSON", content, f"{export_tid}.json", "application/json")
        else:
            st.download_button("Download CSV", data, f"{export_tid}.csv", "text/csv")
        st.code(content if fmt == "json" else data, language="json" if fmt == "json" else None)


    st.subheader("Import Template")
    uploaded_template = st.file_uploader("Upload Template JSON", type=["json"], key="import_file")
